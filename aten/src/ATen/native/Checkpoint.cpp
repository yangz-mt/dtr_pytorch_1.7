...
Tensor checkpoint_gelu(const Tensor& self) {
  // printf("Here is checkpoint_gelu!\n");
  rematerialize_function_t rt = 
    [=](const Tensors& vec) -> Tensors {
    return {at::gelu(vec.at(0))};
  };
  return CheckpointTensorImpl::make("gelu", rt, {self})[0];
}

Tensor checkpoint_gelu_backward(const Tensor& grad, const Tensor& self) {
  // printf("Here is checkpoint_gelu_backward!\n");
  rematerialize_function_t rt = 
    [=](const Tensors& vec) -> Tensors {
    return {at::gelu_backward(vec.at(0), vec.at(1))};
  };
  return CheckpointTensorImpl::make("gelu_backward", rt, {self})[0];
}

Tensor checkpoint_norm_ScalarOpt_dim(const Tensor& self, c10::optional<Scalar> p, IntArrayRef dim, bool keepdim) {
  std::vector<long> dim_ = dim.vec();
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
    return {at::norm(vec.at(0), p, dim_, keepdim)};
  };
  return CheckpointTensorImpl::make("norm_ScalarOpt_dim", rt, {self})[0];
}

Tensor checkpoint_reciprocal(const Tensor& self) {
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
    return {at::reciprocal(vec.at(0))};
  };
  return CheckpointTensorImpl::make("reciprocal", rt, {self})[0];
}

Tensor& checkpoint_reciprocal_(Tensor& self) {
  mutate_function_t mt =
    [=](const Tensors& vec) {
      Tensor self_ = vec.at(0);
      at::reciprocal_(self_);
    };
  CheckpointTensorImpl::mutate("reciprocal_", mt, {self}, {0});
  return self;
}

Tensor& checkpoint_reciprocal_out(Tensor& out, const Tensor& self) {
  mutate_function_t mt =
    [=](const Tensors& vec) {
      Tensor out_ = vec.at(0);
      at::reciprocal_out(out_, vec.at(1));
    };
  CheckpointTensorImpl::mutate("reciprocal_out", mt, {out, self}, {0});
  return out;
}
